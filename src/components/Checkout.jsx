...
export default function Checkout(){
    ...
    const {data, isLoading: isSending, error, sendRequest, clearData} = useHttp("http://localhost:3000/orders", initialConfig);
    ...
    function handleSubmit(event){
        ...
        let actions = (<>
            ...
            <Button>Submir Order</Button>
        </>);

        if(isSending){
           actions = <span>Sending data on progress.</span>
        }

        if(data && !error){
            return <Modal open={userProgressContext.progress === "checkout"} onClose={handleCloseButton}>
                <h2>Success !!.</h2>
